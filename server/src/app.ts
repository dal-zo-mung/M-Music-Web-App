...
import {
  apiReadLimiter,
  apiWriteLimiter,
} from "./modules/shared/middleware/rate-limit.js";
import {
  ensureCsrfTokenCookie,
  rejectDangerousRequestKeys,
} from "./modules/shared/middleware/request-security.js";
...
export function createApp() {
  ...
  app.use(
    cors({
      ...
      origin(origin, callback) {
        ...
        if (env.corsAllowedOrigins.has(origin)) {
          callback(null, true);
          return;
        }
  ...
  const sessionStore = env.mongodbUrl
    ? MongoStore.create({
        autoRemove: "native",
  ...
  if (fs.existsSync(clientDistDir)) {
    app.use(
      express.static(clientDistDir, { maxAge: env.isProduction ? "7d" : 0 }),
    );
  }

  if (fs.existsSync(legacyPublicDir)) {
    app.use(
      express.static(legacyPublicDir, { maxAge: env.isProduction ? "7d" : 0 }),
    );
  }
