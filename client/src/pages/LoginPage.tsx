...
export function LoginPage(): React.JSX.Element {
  ...
  const [messageTone, setMessageTone] = useState<'error' | 'info' | 'success' | 'warning'>('info');
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!passwordValidation.valid) {
      setMessage(`Password format is weak: ${passwordValidation.errors.join('; ')}`);
    ...
    try {
      ...
      const normalizedError =
        error instanceof ApiError ? (error as ApiError<ApiErrorResponse>) : new ApiError('Login failed.', 500, null);
  ...
  return (
    <main className="auth-page">
      <section className="auth-card auth-card--login">
        ...
        {oauthNotice ? <p className="form-message form-message--error">{oauthNotice}</p> : null}
          ...
          <label className="field">
            ...
            <div className="field__input">
              ...
              >
                <img
                  alt=""
                  src={isPasswordVisible ? '/icons/visibility.svg' : '/icons/visibility_off.svg'}
                />
              </button>
            </div>
          </label>
        ...
        <p className="auth-card__switch">
          New here?{' '}
          <Link to={`/register?returnTo=${encodeURIComponent(returnTo)}`}>Create an account</Link>
        </p>
      </section>
    </main>
  );
