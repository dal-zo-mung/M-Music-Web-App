...
const usernameOrEmailSchema = z.string().min(3).max(100).refine((val) => {
...
export const registerSchema = z.object({
  body: z.object({
    username: z.string().min(3).max(30).regex(/^(?=.{3,30}$)[a-z0-9._-]+$/),
...
export const profileUpdateSchema = z.object({
  body: z.object({
    ...
    accentKey: z.enum(['default', 'aurora', 'ember', 'meadow', 'slate']).optional()
  })
});
