    ...
    <main className="search-page">
      <section className="search-shell">
        <div className="search-summary-card">
          ...
          {categoryLabel ? (
            <div className="category-filter-chip">
              <span className="category-filter-chip__label">{categoryLabel}</span>
              <button
        ...
        <div aria-live="polite" className="search-results">
          {isLoading ? <p className="empty-panel">Searching for songs...</p> : null}

          {!isLoading && !searchUrl ? (
            <p className="empty-panel">Search by song title or artist, or pick a category above.</p>
          ) : null}
          ...
          {searchResults.map((song) => (
            <Link className="search-result-card" key={song._id} to={buildSongPath(song._id, query)}>
              <h2>{song['Song Title'] || 'Untitled song'}</h2>
              <p>{[song.Artist, song['About Song']].filter(Boolean).join(' · ')}</p>
            </Link>
          ))}
        </div>
      </section>
