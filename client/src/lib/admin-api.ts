...
export async function deleteAdminUser(
  userId: string,
): Promise<{ success: boolean; deleted: unknown }> {
  return deleteJson<{ success: boolean; deleted: unknown }>(
...
): Promise<{ success: boolean; song: AdminSongRecord }> {
...
export async function fetchAdminCommunity(
  ...
  const query = new URLSearchParams(params).toString();
  return fetchJson<AdminCommunitySubmissionsResponse>(
    `/api/admin/community/submissions?${query}`,
  );
}
...
export async function updateAdminSubmissionStatus(
  submissionId: string,
  status: string,
): Promise<{ success: boolean; submission: CommunitySubmissionRecord }> {
...
export async function updateAdminCommentStatus(
  commentId: string,
  status: string,
): Promise<{ success: boolean; comment: CommunityCommentRecord }> {
