...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  const [
    ...
    CommunitySubmissionModel.countDocuments({
      createdAt: mongoose.trusted({ $gte: sevenDaysAgo }),
    }).exec(),
  ...
  for (const user of allUsers) {
    ...
    usersByProvider[hasGoogle ? "google" : "local"] =
      (usersByProvider[hasGoogle ? "google" : "local"] ?? 0) + 1;
  }
...
export async function listUsers(query: {
  ...
  if (
    ...
    ["admin", "member", "moderator", "user"].includes(query.role)
  ) {
...
export async function getUserDetail(userId: string): Promise<AdminUserRecord> {
  ...
  const [favoriteCount, communityPostCount, communityCommentCount] =
    await Promise.all([
...
export async function listSongsAdmin(query: {
  ...
  if (
    ...
    ["published", "removed"].includes(query.status)
  ) {
  ...
  return {
    ...
    songs: songs.map((song) =>
      toAdminSongRecord(song, favoriteMap.get(String(song._id)) ?? 0),
    ),
    success: true,
  };
}

export async function getSongDetailAdmin(
  songId: string,
...
export async function createSong(
  payload: AdminSongCreateRequest,
): Promise<AdminSongRecord> {
  const title =
    typeof payload["Song Title"] === "string"
      ? payload["Song Title"].trim()
      : "";
  const artist =
  ...
  const song = await SongModel.create({
    "About Song":
      typeof payload["About Song"] === "string"
        ? payload["About Song"].trim()
        : "",
    "Direct to YT":
      typeof payload["Direct to YT"] === "string"
        ? payload["Direct to YT"].trim()
        : "",
    "Released Date":
      typeof payload["Released Date"] === "string"
        ? payload["Released Date"].trim()
        : "",
    "Song Title": title,
    Artist: artist,
...
export async function updateSong(
  ...
  if (
    ...
    ["published", "removed"].includes((payload as any).status)
  ) {
...
export async function deleteSong(
  songId: string,
): Promise<{ favoritesRemoved: number; songId: string }> {
  if (!mongoose.isValidObjectId(songId)) {
    throw httpError("Invalid song identifier.", 400);
  }
...
export async function updateSubmissionStatus(
  ...
  const submission =
    await CommunitySubmissionModel.findById(submissionId).exec();
