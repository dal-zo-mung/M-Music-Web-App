...
export async function getDesktopUser(
  rawDeviceId: unknown,
): Promise<DesktopUserProfile | null> {
  const deviceId = validateDeviceId(rawDeviceId);
 ...
 */
export function initGoogleAuth(rawDeviceId: unknown): {
  authUrl: string;
  token: string;
} {
  const deviceId = validateDeviceId(rawDeviceId);
  ...
  googleAuthUrl.searchParams.set(
    "redirect_uri",
    `${callbackUrl.replace("/auth/google/callback", "")}/api/desktop/auth/google/callback`,
  );
 ...
 */
export function pollGoogleAuth(token: unknown): {
 ...
 */
export async function linkGoogleToGuest(
  rawDeviceId: unknown,
