...
function completeLogout(req: Request, res: Response, next: NextFunction, options: { json?: boolean } = {}): void {
  ...
  req.logout((logoutError) => {
    if (logoutError) {
      console.warn('Passport logout (continuing to destroy session):', logoutError);
    }
  ...
  if (isSafeReturnPath(requestedReturnTo)) {
    ...
    try {
      ...
      if (refererUrl.origin === origin) {
        returnTo = resolveSafeReturnTo(`${refererUrl.pathname}${refererUrl.search}`, '');
      }
    } catch {
      returnTo = '';
    }
...
export function finishGoogleAuth(req: Request, res: Response): void {
  ...
  if (typeof req.query.state === 'string') {
    try {
      redirectTo = resolveSafeReturnTo(decodeURIComponent(req.query.state), redirectTo);
    } catch {
...
export function logoutJson(req: Request, res: Response, next: NextFunction): void {
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const user = await UserModel.findOne(mongoose.trusted({
      $or: [
        { username: username },
