...
import { DesktopSongModel, type DesktopSongPersistenceRecord } from './desktop-song.model.js';
...
export async function listPublicDesktopSongs(query: DesktopSongQuery): Promise<DesktopSong[]> {
...
export async function getPublicDesktopSongById(id: string): Promise<DesktopSong> {
...
export async function listPublicCategories(): Promise<DesktopCategory[]> {
  const categories = await DesktopCategoryModel.find({}).sort({ name: 1 }).lean();
...
export async function getLatestRelease(platform?: string): Promise<Release | null> {
  const filter: Record<string, unknown> = { is_active: true };
  if (platform && ['win', 'mac', 'linux'].includes(platform)) {
    filter.$or = [{ platform }, { platform: 'all' }];
  }

  const doc = await ReleaseModel.findOne(mongoose.trusted(filter) as any).sort({ createdAt: -1 }).exec();
...
export async function adminListDesktopSongs(query: Record<string, unknown>): Promise<{
  songs: DesktopSong[];
  ...
  const [docs, totalItems] = await Promise.all([
    DesktopSongModel.find(mongoose.trusted(filter)).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
...
export async function adminUpdateDesktopSong(
  ...
  if (data.thumbnail_color !== undefined) update.thumbnail_color = data.thumbnail_color;

  const doc = await DesktopSongModel.findByIdAndUpdate(id, { $set: update }, { new: true }).lean();
...
export async function adminDeleteDesktopSong(id: string): Promise<void> {
  ...
  if (result.deletedCount === 0) throw httpError('Desktop song not found.', 404);
}
...
export async function adminListCategories(): Promise<DesktopCategory[]> {
  ...
  const countMap = Object.fromEntries(counts.map((c) => [c._id, c.count]));
  return categories.map((cat) => ({ ...toDesktopCategory(cat), song_count: countMap[cat.name] ?? 0 }));
}

export async function adminCreateCategory(name: string, language?: string): Promise<DesktopCategory> {
  if (!name?.trim()) throw httpError('Category name is required.', 400);
  const doc = await DesktopCategoryModel.create({ name: name.trim(), language });
...
export async function adminUpdateCategory(
  ...
  const doc = await DesktopCategoryModel.findByIdAndUpdate(id, { $set: update }, { new: true }).lean();
...
export async function adminCreateRelease(data: {
  ...
  if (!data.download_url?.trim()) throw httpError('Download URL is required.', 400);
