...
export async function listCommunityFeedHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
...
export async function getCommunitySubmissionDetailsHandler(
  ...
  try {
    const detail = await getCommunitySubmissionDetails(
...
export async function createCommunitySubmissionHandler(
  ...
  try {
    res
      .status(201)
      .json(await createCommunitySubmission(req.body, req.currentUser!));
  } catch (error) {
    next(error);
  }
}
